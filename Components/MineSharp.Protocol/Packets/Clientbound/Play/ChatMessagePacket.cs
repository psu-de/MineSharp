...
{
    public static int Id => 0x35;

    public IChatMessageBody Body { get; set; }

    public ChatMessagePacket(IChatMessageBody body)
    {
        this.Body = body;
    }
    ...
    public static IPacket Read(PacketBuffer buffer, MinecraftData version, string packetName)
    {
        if (version.Protocol.Version <= ProtocolVersion.V_18_2)
            return new ChatMessagePacket(V1_18Body.Read(buffer));

        if (version.Protocol.Version == ProtocolVersion.V_1_19)
            return new ChatMessagePacket(V1_19Body.Read(buffer));

        if (version.Protocol.Version >= ProtocolVersion.V_1_19_2)
            return new ChatMessagePacket(V1_19_2_3Body.Read(buffer, version));
    ...
    public class V1_18Body : IChatMessageBody
    {
        public UUID Sender { get; set; }
        public string Message { get; set; }
        public byte Position { get; set; }

        public V1_18Body(UUID sender, string message, byte position)
        {
            this.Sender = sender;
        ...
        public static V1_18Body Read(PacketBuffer buffer)
        {
            var sender = buffer.ReadUuid();
            var message = buffer.ReadString();
            var position = buffer.ReadByte();
            return new V1_18Body(sender, message, position);
        }
    }
    ...
    public class V1_19_2_3Body : IChatMessageBody
    {
        ...
        public long Timestamp { get; set; }
        public long Salt { get; set; }
        public Serverbound.Play.ChatMessagePacket.V1_19.MessageItem[] PreviousMessages { get; set; }
        ...
        private V1_19_2_3Body(byte[]? previousSignature, UUID sender, int? index, byte[]? signature, string plainMessage, string? formattedMessage, long timestamp, long salt, Serverbound.Play.ChatMessagePacket.V1_19.MessageItem[] previousMessages, string? unsignedContent, int filterType, long[]? filterTypeMask, int type, string networkName, string? networkTargetName)
        {
        ...
        public V1_19_2_3Body(byte[]? previousSignature, UUID sender, byte[] signature, string plainMessage, string? formattedMessage, long timestamp, long salt, Protocol.Packets.Serverbound.Play.ChatMessagePacket.V1_19.MessageItem[] previousMessages, string? unsignedContent, int filterType, long[]? filterTypeMask, int type, string networkName, string? networkTargetName)
        {
            this.PreviousSignature = previousSignature;
            this.Sender = sender;
            this.Signature = signature;
        ...
        public V1_19_2_3Body(
            ...
            long timestamp,
            long salt,
            Serverbound.Play.ChatMessagePacket.V1_19.MessageItem[] previousMessages,
        ...
        public void Write(PacketBuffer buffer, MinecraftData data)
        {
            if (data.Protocol.Version == ProtocolVersion.V_1_19_2)
            {
        ...
        public static V1_19_2_3Body Read(PacketBuffer buffer, MinecraftData version)
        {
            ...
            previousMessages = buffer.ReadVarIntArray(Serverbound.Play.ChatMessagePacket.V1_19.MessageItem.Read);
