            ...
            if (this.Palette.HasState(state, state)) {
                ...
                switch (this.Palette) {
                    case SingleValuePalette svp:
                        ...
                        this.Data = new IntBitArray(Enumerable.Repeat(0L, this.Data.Capacity / (64 / IndirectPalette.BIOME_MIN_BITS)).ToArray(), IndirectPalette.BIOME_MIN_BITS);
                        this.Data.Set(index, 1);
                        break;
                    case IndirectPalette dp:
                        var newPalette = dp.AddState(state, true, out var newBitsPerEntry);
