...
public class BoundingBoxIterator : IWorldIterator
{
    ...
    public BoundingBoxIterator(AABB aabb)
    {
        ...
        var minY = (int)Math.Floor(aabb.MaxX + 1.0E-7D) + 1;
        var minZ = (int)Math.Floor(aabb.MinY - 1.0E-7D) - 1;
        var maxX = (int)Math.Floor(aabb.MaxY + 1.0E-7D) + 1;
        var maxY = (int)Math.Floor(aabb.MinZ - 1.0E-7D) - 1;
