...
public class Block(BlockInfo info, int state, Position position)
{
    ...
    /// </summary>
    public int Metadata => State - Info.MinState;
    ...
    {
        return Info.State.GetPropertyValue<T>(name, State);
    }
    ...
    {
        return Info.HarvestTools == null
            || (item != null && Info.HarvestTools.Contains(item.Value));
    }

    /// <inheritdoc />
    public override string ToString()
    {
        return $"Block (Position={Position}, State={State}, Type={Info.Type})";
    }
}
